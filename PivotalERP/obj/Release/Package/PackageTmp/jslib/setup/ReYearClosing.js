app.controller('ReYearClosingController', function ($scope, $http, $timeout, $filter, GlobalServices) {
	...
	$scope.CallSaveUpdateYearClosing = function () {
		...
		Swal.fire({
			...
			if (result.isConfirmed)
			{
				...
				$http({
					method: 'POST',
					url: base_url + "Setup/Security/DoReYearClosing",
	...
	$scope.ClearStudentOpening = function (aid) {

		Swal.fire({
			title: 'Are You Sure To Delete Student Opening For Selected Academic Year ? ',
			showCancelButton: true,
			confirmButtonText: 'Yes',
		}).then((result) => { 
			if (result.isConfirmed) {
				...
				var para = {
					AcademicYearId: aid,
					Pwd: $scope.newYearClosing.Pwd
                }
				$http({
					method: 'POST',
					url: base_url + "Setup/Security/ClearStudentOpening",
					dataType: "json",
