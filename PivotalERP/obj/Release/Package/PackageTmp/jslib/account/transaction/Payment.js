app.controller('PaymentController',  function ($scope, $http, $timeout, $filter, GlobalServices) {
    ...
    $scope.getVoucherNo = function () {
        ...
        if ($scope.beData.VoucherId && $scope.beData.VoucherId > 0 && $scope.beData.TranId==0)
        {
            if ($scope.beData.CostClassId && $scope.beData.CostClassId > 0) {
                var para = {
                    voucherId: $scope.beData.VoucherId,
                ...
                if (ledDet.LedgerId && ledDet.LedgerId > 0 && ledDet.LedgerDetails) {
                    ...
                    if (ledDet.LedgerDetails.BillWiseAdjustment == true) {
                        ...
                        if (!ledDet.BillRefColl || ledDet.BillRefColl.length == 0) {
                            $http({
                                method: 'GET',
                                url: base_url + "Inventory/Transaction/getPurchaseDuesBillList?ledgerId=" + ledDet.LedgerId,
                                dataType: "json"
                            }).then(function (res) {
                                if (res.data.IsSuccess && res.data.Data) {
    ...
    $scope.GetData = function () {
        ...
        {
            vDate = $filter('date')(new Date($scope.beData.VoucherDate), 'yyyy-MM-dd');
        }
