...
namespace AcademicLib.DA.Academic.Creation
{
    internal class CategoryDB
    {
        ...
        public ResponeValues SaveUpdate(BE.Academic.Creation.Category beData, bool isModify)
        {
        ...
        public BE.Academic.Creation.CategoryCollections getAllCategory(int UserId, int EntityId)
        {
            BE.Academic.Creation.CategoryCollections dataColl = new BE.Academic.Creation.CategoryCollections();
            ...
            try
            {
                ...
                while (reader.Read())
                {
                    BE.Academic.Creation.Category beData = new BE.Academic.Creation.Category();
        ...
        public BE.Academic.Creation.Category getCategoryById(int UserId, int EntityId, int CategoryId)
        {
            BE.Academic.Creation.Category beData = new BE.Academic.Creation.Category();
            ...
            try
            {
                ...
                if (reader.Read())
                {
                    beData = new BE.Academic.Creation.Category();
                    beData.CategoryId = reader.GetInt32(0);
                    if (!(reader[1] is DBNull)) beData.Code = reader.GetString(1);
                    if (!(reader[2] is DBNull)) beData.Name = reader.GetString(2);
                    if (!(reader[3] is DBNull)) beData.DisplayName = reader.GetString(3);
