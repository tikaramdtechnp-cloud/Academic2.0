...
{
    internal class UrineTestDB
    {
        DataAccessLayer1 dal = null;
        public UrineTestDB(string hostName, string dbName)
        {
            dal = new DataAccessLayer1(hostName, dbName);
        }
        public ResponeValues SaveUpdate(AcademicERP.BE.Health.Transaction.UrineTest beData, bool isModify)
        {
            ...
            cmd.Parameters.AddWithValue("@Protein", beData.Protein);
            cmd.Parameters.AddWithValue("@Sugar", beData.Sugar);
            cmd.Parameters.AddWithValue("@Transparency", beData.Transparency);
            cmd.Parameters.AddWithValue("@WBC", beData.WBC);
            ...
            if (isModify)
            {
                cmd.CommandText = "usp_UpdateUrineTest";
            }
            else
            {
                cmd.Parameters[10].Direction = System.Data.ParameterDirection.Output;
                cmd.CommandText = "usp_AddUrineTest";
            }
            ...
            try
            {
                ...
                {
                    SaveAttachDocuments(beData.AttachUrTeColl, resVal.RId, beData.CUserId);
                }
        ...
        private void SaveAttachDocuments(Dynamic.BusinessEntity.GeneralDocumentCollections dataColl, int TranId, int UserId)
        {
            foreach (var beData in dataColl)
            {
                ...
                cmd.CommandText = "usp_AddUrineTestAttDoc";
                cmd.ExecuteNonQuery();
            }
        }
        public AcademicERP.BE.Health.Transaction.UrineTestCollections getAllUrineTest(int UserId, int EntityId, int? StudentId, int? EmployeeId)
        {
            ...
            cmd.CommandText = "usp_GetAllUrineTest";
            try
            {
        ...
        public AcademicERP.BE.Health.Transaction.UrineTest getUrineTestById(int UserId, int EntityId, int TranId)
        {
            AcademicERP.BE.Health.Transaction.UrineTest beData = new AcademicERP.BE.Health.Transaction.UrineTest();
            ...
            cmd.Parameters.AddWithValue("@UserId", UserId);
            cmd.Parameters.AddWithValue("@EntityId", EntityId);
            cmd.CommandText = "usp_GetUrineTestById";
            try
            {
                System.Data.SqlClient.SqlDataReader reader = cmd.ExecuteReader();
                if (reader.Read())
                {
                    beData = new AcademicERP.BE.Health.Transaction.UrineTest();
                    beData.TranId = reader.GetInt32(0);
                    if (!(reader[1] is DBNull)) beData.TestDate = reader.GetDateTime(1);
                    if (!(reader[2] is DBNull)) beData.Color = reader.GetString(2);
                    if (!(reader[3] is DBNull)) beData.Protein = reader.GetString(3);
        ...
        public ResponeValues DeleteById(int UserId, int EntityId, int TranId)
        {
            ...
            cmd.Parameters.AddWithValue("@EntityId", EntityId);
            cmd.Parameters.AddWithValue("@TranId", TranId);
            cmd.CommandText = "usp_DelUrineTestById";
            cmd.Parameters.Add("@ResponseMSG", System.Data.SqlDbType.NVarChar, 254);
            cmd.Parameters.Add("@IsSuccess", System.Data.SqlDbType.Bit);
            cmd.Parameters.Add("@ErrorNumber", System.Data.SqlDbType.Int);
